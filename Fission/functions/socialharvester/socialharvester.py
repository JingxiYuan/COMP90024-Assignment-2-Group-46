...
import warnings,ssl,requests,json
...
def main():
    ...
    for i in range(0, len(social_response)):
        social_response[i]["created_at"] = parse_time(social_response[i])
        text = parse_content(social_response[i])
        social_response[i]["content"] = text
        social_response[i]["sentiment"] = get_sentiment(text)

    upload = []
    temp = ["created_at", "content", "sentiment", "id"]
    for i in social_response:
        temp_dict = {k: v for k, v in i.items() if k in temp}
        upload.append(temp_dict)

    current_app.logger.info(f"Harvested {len(upload)} From Mastodon's Social server")
    requests.post(url='http://router.fission/enqueue/mastodonsocial',
                  headers={'Content-Type': 'application/json'},
                  data=json.dumps({"toots": upload})
                  )
    return f"Harvested {len(upload)} From Mastodon's Social server"