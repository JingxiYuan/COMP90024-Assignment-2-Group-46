...
def main():
    ...
    au_crime_response = au_mast.timeline_hashtag(hashtag="crime", limit=40)

    for i in range(0, len(au_crime_response)):
        au_crime_response[i]["created_at"] = parse_time(au_crime_response[i])
        text = parse_content(au_crime_response[i])
        au_crime_response[i]["content"] = text
        au_crime_response[i]["sentiment"] = get_sentiment(text)

    upload = []
    temp = ["created_at", "content", "sentiment", "id"]
    for i in au_crime_response:
    ...
    requests.post(url='http://router.fission/enqueue/crime',
