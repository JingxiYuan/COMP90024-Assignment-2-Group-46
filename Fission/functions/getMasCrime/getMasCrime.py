...
def es_download_specific_crime_fields(es, index):
    query = {
        "query": {
            "match_all": {}  
        },
...
def main():
    ...
    indices_info = es.indices.get_alias(name="mastodon_crime*")
    indices = [index for index in indices_info]

    all_crime_data = []
    for index in indices:
        data = es_download_specific_crime_fields(es, index)
        all_crime_data.extend(data)
    
    # Convert combined_data to a JSON string
    json_string = json.dumps(all_crime_data)
    print("Total number of records retrieved:", len(all_crime_data))
    print("done")
    return json_string